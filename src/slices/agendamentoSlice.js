...
export const fetchAgendamentosAssistente = createAsyncThunk(
  "agendamento/fetchAllAssistente",
  async (userId, thunkAPI) => {
    try {
      const data = await agendamentoService.getAgendamentosAssitente(userId);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.message);
    }
...
export const createAgendamentoAssistente = createAsyncThunk(
  "agendamento/createAssistente",
  async (agendamentoData, thunkAPI) => {
    try {
      const data = await agendamentoService.createAgendamentoAssistente(
        agendamentoData
      );
      return data;
    } catch (error) {
...
export const updateAgendamento = createAsyncThunk(
  ...
  async ({ id, agendamentoData }, thunkAPI) => {
    try {
      const data = await agendamentoService.updateAgendamento(
        id,
        agendamentoData
      );
      return data;
...
export const associatePacienteToAgendamento = createAsyncThunk(
  ...
  async ({ agendamentoId, pacienteId }, thunkAPI) => {
    try {
      const data = await agendamentoService.associatePacienteToAgendamento(
        agendamentoId,
        pacienteId
      );
      return data;
...
export const agendamentoSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAgendamentosAssistente.pending, (state) => {
        state.loading = true;
        state.error = false;
      })
      .addCase(fetchAgendamentosAssistente.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        state.agendamentosAssistente = action.payload;
      })
      .addCase(fetchAgendamentosAssistente.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(createAgendamentoAssistente.pending, (state) => {
        state.loading = true;
        state.error = false;
      })
      .addCase(createAgendamentoAssistente.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        state.agendamentosAssistente.push(action.payload);
        state.message = "Agendamento criado com sucesso!";
      })
      .addCase(createAgendamentoAssistente.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
