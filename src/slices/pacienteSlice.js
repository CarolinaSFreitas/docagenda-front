...
export const fetchPacientePorMedico = createAsyncThunk(
  "paciente/fetchByMedico",
  async (userId, thunkAPI) => {
    try {
      const data = await pacienteService.getPacientePorMedico(userId);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.message);
    }
...
export const createPacienteAssistente = createAsyncThunk(
  "paciente/createAssistente",
  async (pacienteData, thunkAPI) => {
    try {
      const data = await pacienteService.createPacienteComMedico(pacienteData);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.message);
    }
...
export const searchPacienteAssistente = createAsyncThunk(
  "paciente/searchAssistente",
  async ({ q, userId }, thunkAPI) => {
    try {
      const data = await pacienteService.searchPacienteAssistente(q, userId);
      return data;
    } catch (error) {
      return thunkAPI.rejectWithValue(error.message);
    }
...
const pacienteSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchPacientePorMedico.pending, (state) => {
        state.loading = true;
        state.error = false;
      })
      .addCase(fetchPacientePorMedico.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        state.pacientePorMedico = action.payload;
      })
      .addCase(fetchPacientePorMedico.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
      ...
      .addCase(searchPacienteAssistente.pending, (state) => {
        state.loading = true;
        state.error = false;
      })
      .addCase(searchPacienteAssistente.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        state.pacientes = action.payload;
      })
      .addCase(searchPacienteAssistente.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      })
