...
const getPacientePorMedico = async (userId) => {
  const token = getAuthToken();
  const config = requestConfig("GET", null, token);

  try {
    const res = await fetch(api + `/paciente/medico/${userId}`, config)
      .then((res) => res.json())
      .catch((err) => err);
...
const createPacienteComMedico = async (data) => {
  const token = getAuthToken();
  const config = requestConfig("POST", data, token);

  try {
    const res = await fetch(api + "/paciente/assistente", config)
      .then((res) => res.json())
      .catch((err) => err);
...
const searchPacienteAssistente = async (query, userId) => {
  const token = getAuthToken();
  const config = requestConfig("GET", null, token);

  try {
    const res = await fetch(
      api + `/paciente/searchAssistente?q=${query}&userId=${userId}`,
      config
    )
      .then((res) => res.json())
      .catch((err) => err);
...
const downloadAnexo = async (pacienteId, exameId) => {
  ...
  try {
    const res = await fetch(
      api + `/paciente/${pacienteId}/exames/${exameId}/download`,
      config
    )
      .then((res) => res.blob())
      .catch((err) => err);
...
const viewAnexo = async (pacienteId, exameId) => {
  ...
  try {
    const res = await fetch(
      api + `/paciente/${pacienteId}/exames/${exameId}/view`,
      config
    )
      .then((res) => res.blob())
      .then((blob) => {
...
const deleteExame = async (pacienteId, exameId) => {
  ...
  try {
    const res = await fetch(
      api + `/paciente/${pacienteId}/exames/${exameId}`,
      config
    )
      .then((res) => res.json())
      .catch((err) => err);
