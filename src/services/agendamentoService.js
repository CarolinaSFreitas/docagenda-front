...
const getAgendamentosAssitente = async (userId) => {
  const token = getAuthToken();
  const config = requestConfig("GET", null, token);

  try {
    const res = await fetch(api + `/agendamento/assistente/${userId}`, config)
      .then((res) => res.json())
      .catch((err) => err);
...
const createAgendamentoAssistente = async (data) => {
  const token = getAuthToken();
  const config = requestConfig("POST", data, token);

  try {
    const res = await fetch(api + "/agendamento/assistente", config)
      .then((res) => res.json())
      .catch((err) => err);
...
const associatePacienteToAgendamento = async (agendamentoId, pacienteId) => {
  ...
  try {
    const res = await fetch(
      api + `/agendamento/${agendamentoId}/paciente/${pacienteId}`,
      config
    )
      .then((res) => res.json())
      .catch((err) => err);
